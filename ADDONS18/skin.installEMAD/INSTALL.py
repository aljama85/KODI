...
def UPDATE_REPOS(delay=10):
	xbmc.executebuiltin('UpdateAddonRepos')
	xbmc.log('skin.installEMAD ========= Executed UpdateAddonRepos',level=xbmc.LOGNOTICE)
	time.sleep(delay)
	return
...
def updateProgressBar(dialog22,window22):
	...
	for ii in range(progressbar_totaltime):
		...
		if ii%30==28:
			...
			elif 115<time_new-time_old<125: break
		if ii%30==2:
...
"""
INSTALL_ADDON('plugin.video.arabicvideos')
version = ADDON_VERSION('plugin.video.arabicvideos')
SHOW_AV_VERSION(window,dialog.lang,version)

INSTALL_ADDON('skin.metropolisEMAD')
version = ADDON_VERSION('skin.metropolisEMAD')
SHOW_SKIN_VERSION(window,dialog.lang,version)

INSTALL_ADDON('resource.language.ar_sa')
"""
